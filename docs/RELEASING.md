...
4. Confirm normal CI and all six dry-run packaging jobs pass on the release pull request.
5. Merge the release pull request. Its closed-and-merged event creates the matching tag and GitHub Release from the resulting `main` commit. Fork pull requests and ordinary branch names cannot publish.
6. Confirm all six release packaging jobs pass and the GitHub release contains six archives plus `checksums.txt`.
7. Download at least one archive, verify its checksum, run `--version`, and generate a JSON report without elevated privileges.
8. Confirm the website and installation documentation link to the new release.
